# Design: Upgrade reth Dependency to v2.3.0

## Background

Upgrade reth-remote-exex's reth dependency from `dev-exex-internal-txs` (v2.2.0) to `dev-exex-internal-txs-v2.3.0` (v2.3.0).
...
|------|----------------|--------|
| `Cargo.toml` | Must change | Switch git branch; may need to upgrade alloy/revm versions to match reth v2.3.0 dependency tree |
| `src/convert.rs` | May change | If reth/alloy types have breaking changes (field renames, method signature changes) |
| `src/exex.rs` | May change | If ExExContext/ExExNotification/CanonStateNotification APIs changed |
...
```
reth-remote-exex
├── reth (git, branch: dev-exex-internal-txs-v2.3.0)  ← changed
...
| B: Pre-analyze upstream diff | Compare reth v2.2.0 vs v2.3.0 API differences, plan changes in advance | Overkill for a small project; reth codebase is huge, diff signal-to-noise ratio is low |
...
- reth v2.3.0 may introduce new `ExExNotification` variants or `AccountStatus` enum values requiring match arm additions
...
| T2 | Unit tests pass | `cargo test` | All 9 existing tests pass |
...
- **T2**: Verifies flag filtering logic in convert.rs behaves identically under new dependencies (9 tests cover all flag combinations)
