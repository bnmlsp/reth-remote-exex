# Requirements: Upgrade reth Dependency to v2.4.0

## Background

The project currently depends on the custom fork `bnmlsp/reth`, branch `dev-exex-internal-txs-v2.3.0` (reth v2.3.0). An upstream branch `dev-exex-internal-txs-v2.4.0` (based on reth v2.4.0) is now available. The project needs to upgrade to the new branch to track the upstream version.

## Goal

Upgrade reth-remote-exex's reth dependency from v2.3.0 (branch `dev-exex-internal-txs-v2.3.0`) to v2.4.0 (branch `dev-exex-internal-txs-v2.4.0`), ensuring the project compiles, all tests pass, and deployed functionality is not degraded.

## Functional Requirements

| ID | Requirement | Priority |
|----|-------------|----------|
| R1 | Switch all `bnmlsp/reth` git dependencies in Cargo.toml from branch `dev-exex-internal-txs-v2.3.0` to `dev-exex-internal-txs-v2.4.0` | Must |
| R2 | Fix all compilation errors caused by reth v2.4.0 API changes so that `cargo build --release` succeeds | Must |
...
- **Given** Cargo.toml reth dependencies have been switched to the `dev-exex-internal-txs-v2.4.0` branch
...
- No functional changes leveraging reth v2.4.0 new APIs
- No new tag (until deployment is verified)
- No modifications to code unrelated to the upgrade
- No upgrading alloy or other dependencies unless forced by reth v2.4.0
- Base chain support (handled separately on feat-base-support)

## Dependencies

- Upstream branch `dev-exex-internal-txs-v2.4.0` (`https://github.com/bnmlsp/reth.git`) is ready

## Assumptions and Constraints

- The upstream fork's `dev-exex-internal-txs-v2.4.0` branch includes the same ExEx internal txs feature extensions as `dev-exex-internal-txs-v2.3.0`
