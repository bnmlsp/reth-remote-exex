...
const (
	...
	chCapacity = 32
	// errChCapacity matches the number of subscriber goroutines so each can send
	// without blocking if both fail simultaneously.
	errChCapacity = 2
)
...
type arrival struct {
	...
	txCount     uint64 // number of transactions (gRPC side only; 0 on WS side)
	gasUsed     uint64 // gas used in block (gRPC side only; 0 on WS side)
}
...
func main() {
	...
	rpcURL   := flag.String("rpc", defaultRPC, "HTTP JSON-RPC endpoint for debug_traceBlockByNumber")
	blocks   := flag.Int("blocks", defaultBlocks, "number of matched blocks before exit")
	flag.Parse()
	...
	for {
		...
		case a := <-grpcCh:
			...
			p.gasUsed = a.gasUsed
			if p.rpcMs != 0 {
				matched, unmatched, deltas = emit(a.blockNumber, p.txCount, p.gasUsed, p.grpcMs, p.rpcMs, matched, unmatched, deltas)
		...
		case a := <-rpcCh:
			...
			if p.grpcMs != 0 {
				// p.txCount and p.gasUsed were written by the grpcCh case above.
				matched, unmatched, deltas = emit(a.blockNumber, p.txCount, p.gasUsed, p.grpcMs, p.rpcMs, matched, unmatched, deltas)
				delete(pending, a.blockNumber)
				if matched == target {
					printStats(matched, unmatched, deltas)
...
func emit(blockNum, txCount, gasUsed uint64, grpcMs, rpcMs int64, matched, unmatched int, deltas []int64) (int, int, []int64) {
	deltaMs := grpcMs - rpcMs
	sign := "+"
	if deltaMs < 0 {
		sign = ""
	}
	fmt.Printf("block=%d txs=%d gas=%d grpc=%dms rpc=%dms delta=%s%dms\n", blockNum, txCount, gasUsed, grpcMs, rpcMs, sign, deltaMs)
