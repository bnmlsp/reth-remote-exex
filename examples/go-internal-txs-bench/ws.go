...
// for each new block immediately calls debug_traceBlockByNumber on rpcURL. The
...
func SubscribeWS(ctx context.Context, wsURL, rpcURL string, ch chan<- arrival, errCh chan<- error) {
	...
	for {
		...
		arrivedAt, err := fetchBlockTraces(ctx, rpcURL, blockNum)
		if err != nil {
			if ctx.Err() != nil {
				return
			}
			errCh <- fmt.Errorf("debug_traceBlockByNumber block=%d: %w", blockNum, err)
			return
		}
...
// fetchBlockTraces calls debug_traceBlockByNumber for the given block number
// and returns the time (Unix millis) immediately after the HTTP response returns.
func fetchBlockTraces(ctx context.Context, rpcURL string, blockNum uint64) (int64, error) {
	hexNum := fmt.Sprintf("0x%x", blockNum)
	body := fmt.Sprintf(`{"jsonrpc":"2.0","id":1,"method":"debug_traceBlockByNumber","params":["%s",{"tracer":"callTracer"}]}`, hexNum)
