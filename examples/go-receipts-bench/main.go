...
func main() {
	...
	rpcURL   := flag.String("rpc", defaultRPC, "HTTP JSON-RPC endpoint for eth_getBlockReceipts")
	...
	for {
		...
		case a := <-grpcCh:
			...
			if p.rpcMs != 0 {
				matched, unmatched, deltas = emit(a.blockNumber, p.grpcMs, p.rpcMs, matched, unmatched, deltas)
...
func emit(blockNum uint64, grpcMs, rpcMs int64, matched, unmatched int, deltas []int64) (int, int, []int64) {
	deltaMs := grpcMs - rpcMs
	sign := "+"
	if deltaMs < 0 {
		sign = ""
	}
	fmt.Printf("block=%d grpc=%dms rpc=%dms delta=%s%dms\n", blockNum, grpcMs, rpcMs, sign, deltaMs)
