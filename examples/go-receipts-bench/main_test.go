...
func TestEmit_GrpcFaster(t *testing.T) {
	_, _, deltas := emit(100, 1000, 1020, 0, 0, nil)
	if len(deltas) != 1 {
...
func TestEmit_RpcFaster(t *testing.T) {
	_, _, deltas := emit(100, 1020, 1000, 0, 0, nil)
	if len(deltas) != 1 {
		t.Fatalf("want 1 delta, got %d", len(deltas))
	}
	if deltas[0] != 20 {
...
func TestEmit_Equal(t *testing.T) {
	_, _, deltas := emit(100, 1000, 1000, 0, 0, nil)
	if len(deltas) != 1 {
		t.Fatalf("want 1 delta, got %d", len(deltas))
	}
	if deltas[0] != 0 {
...
func TestEmit_MatchedIncrement(t *testing.T) {
	matched, unmatched, _ := emit(100, 1000, 1000, 5, 2, nil)
	if matched != 6 {
...
func TestEmit_AppendsDelta(t *testing.T) {
	_, _, deltas := emit(1, 100, 110, 0, 0, nil)
	_, _, deltas = emit(2, 200, 190, 1, 0, deltas)
	if len(deltas) != 2 {
