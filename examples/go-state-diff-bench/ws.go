...
// for each new block immediately calls trace_replayBlockTransactions on rpcURL.
// The arrival time is recorded after the HTTP call returns. Errors are sent to
...
func SubscribeWS(ctx context.Context, wsURL, rpcURL string, ch chan<- arrival, errCh chan<- error) {
	...
	var ack wsAck
	if jsonErr := json.Unmarshal(ackMsg, &ack); jsonErr != nil {
		fmt.Fprintf(os.Stderr, "ws: ack parse failed, proceeding: %v\n", jsonErr)
	...
	for {
		...
		arrivedAt, err := fetchBlockStateDiff(ctx, rpcURL, blockNum)
		if err != nil {
			if ctx.Err() != nil {
				return
			}
			errCh <- fmt.Errorf("trace_replayBlockTransactions block=%d: %w", blockNum, err)
			return
		}
...
// fetchBlockStateDiff calls trace_replayBlockTransactions for the given block
// number and returns the time (Unix millis) immediately after the HTTP response
// returns.
func fetchBlockStateDiff(ctx context.Context, rpcURL string, blockNum uint64) (int64, error) {
	hexNum := fmt.Sprintf("0x%x", blockNum)
	body := fmt.Sprintf(`{"jsonrpc":"2.0","id":1,"method":"trace_replayBlockTransactions","params":["%s",["stateDiff"]]}`, hexNum)
